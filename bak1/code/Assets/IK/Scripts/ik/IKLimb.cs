...
public class IKLimb: MonoBehaviour {
	...
	//	Quaternion upperArmRotationBak = upperArm.localRotation;

		//foot penetration
		target.position = hand.position;
		if (target.position.y < handLength) {
			target.position = new Vector3 (hand.position.x, handLength, hand.position.z);
		}
...
//		upperArmAxisCorrection.transform.localRotation = //.x -= ikAngle;
//			Quaternion.Euler(upperArmAxisCorrection.transform.localRotation.eulerAngles - new Vector3(0, ikAngle, 0));
		...
		//upperArm.localRotation = upperArmRotationBak;
