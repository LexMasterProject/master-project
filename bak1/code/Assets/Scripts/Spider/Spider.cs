...
public class Spider : MonoBehaviour
{
	...
	private Status spiderSubStatus;
	private float dangerDist;
	private float safeDangerDist;
	private Vector3 dangerPos;
	private int turnDangerCount;
	private Direction turnDangerHint;
	...
	void resetSystem()
	{
		if (transform.position.y < -10) {
			transform.position=new Vector3(-39.21f,3.68f,-66.55f);
		}
	...
	EnvObjectType EyeRaysDetectEnvObjects(ref Vector3 targetPos)
	{
		Vector3 defaultV = transform.forward;
		Vector3 [] eyeRays=new Vector3[7];
		eyeRays[0]= Quaternion.AngleAxis (-20, transform.up)*defaultV;
		...
		eyeRays[6]= Quaternion.AngleAxis (20, transform.up)*defaultV;
	...
	bool EyeRaysDetectObstacle(ref Direction turnHint,float distOffset)
	{
		...
		for (int i=0; i<eyeRays.Length; i++) {
			dist[i]=1000f;
			if (Physics.Raycast (transform.position, eyeRays[i], out hit, eyeScope)) {
				if(hit.collider.gameObject.tag=="food" || hit.collider.gameObject.tag=="danger" )
								{
									dist[i]=1000f;
								}
				else{
	...
	void spiderStopTurn(Direction turnHint,float s)
	{
	...
	void Update ()
	{
		...
		if (EnvObjectType.DANGER == EyeRaysDetectEnvObjects (ref targetPos)&&spiderStatus!=Status.RUN_AWAY_DANGER) {
			Debug.Log("sense danger");
			spiderStatus=Status.RUN_AWAY_DANGER;
			dangerPos=targetPos;
			turnDangerHint=(Random.Range(0,1)>0.5)?Direction.LEFT:Direction.RIGHT;
			turnDangerCount=Random.Range(150,300);

		
		}

		if (spiderStatus == Status.RUN_AWAY_DANGER) {
			if(Vector3.Distance(dangerPos,transform.position)<safeDangerDist&&spiderSubStatus!=Status.STOP_TURN_DANGER)
			{
				Debug.Log("run away danger");
				anim.SetBool("_isMoving",true);
				anim.SetBool("_isBack",true);
				speed=10;
				changeVelocityInXZ(-transform.forward,speed);
				updateLimbFreq ();
			...
			{
				spiderSubStatus=Status.STOP_TURN_DANGER;
				//quick turn and run
				anim.SetBool("_isBack",false);
				if(turnDangerCount>0)
				{
					Debug.Log("turn");
					speed=0;
					spiderStopTurn(turnDangerHint,0.9f);
					updateLimbFreq ();
					turnDangerCount--;
					return;
				...
				{
					spiderSubStatus=Status.NONE;
				}
			}
		}


		//Debug.Log(anim.GetBool("_isMoving"));
		...
		} else if(obstacleTooClose)
		{
		    ...
		    if(EnvObjectType.FOOD==EyeRaysDetectEnvObjects(ref targetPos))
			{
				spiderStatus= Status.RUN_TO_FOOD;
				runTowards(targetPos);
			}
			else if((lastStatus==Status.STOP_TURN_EDGE||lastStatus==Status.STOP_TURN_PREPARE_EDGE)&&preparationCountEdge>0)
			{
	...
	void normalResponse()
	{
		...
		if (!anim.GetBool ("_isMoving")) {
			...
			if (timeRecord > idleDuration) {
				...
				speedBak=Random.Range(5,10);
			}
			
		} else if (anim.GetBool ("_isMoving")) {
			//moving state
			speed=speedBak;
			changeVelocityInXZ(rb.transform.forward,speed);
			switch(direction)
			{
	...
	void runTowards(Vector3 pos)
	{
		...
		changeVelocityInXZ(pos-transform.position,speed);
		anim.SetBool ("_isMoving", true);
	}
	...
	/*
			 * The Warning will be triggered when A&&B
			 * Condition A:
			 * the spider are too close to the edgeedge
			 * Condition B:
